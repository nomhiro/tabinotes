    ...
    <div style={{ fontFamily: "'Noto Serif JP', 'Hiragino Mincho ProN', serif", background: "#F7F3ED", minHeight: "100vh", color: "#2C2421" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .trip-list-label { font-family:'Zen Maru Gothic',sans-serif; font-size:.8rem; letter-spacing:.5em; color:#6a6058; margin-bottom:1rem; }
        .trip-list-title { font-size:clamp(2rem,5vw,3rem); font-weight:700; letter-spacing:.15em; color:#2C2421; margin:0; }
        ...
        .trip-card:focus-visible { outline:3px solid #2C2421; outline-offset:2px; transform:translateY(-4px); box-shadow:0 12px 40px rgba(0,0,0,.2); }
        ...
        .trip-card-sub { font-size:clamp(.8rem,2vw,1rem); font-weight:300; letter-spacing:.2em; opacity:.9; margin-bottom:1.2rem; }
        ...
        .trip-card-members { margin-top:.8rem; font-size:.78rem; opacity:.85; letter-spacing:.1em; font-family:'Zen Maru Gothic',sans-serif; }
      ...
      <header className="trip-list-header">
        ...
        <h1 className="trip-list-title">旅のしおり</h1>
      </header>
      ...
      <main className="trip-grid">
        {[...trips].sort((a, b) => {
          ...
          <a
            ...
            className="trip-card"
            href={`#/trip/${trip.id}`}
            aria-label={`${trip.title} — ${trip.subtitle}（${trip.dates}）`}
            style={{
              background: `linear-gradient(160deg, ${trip.color} 0%, ${trip.colorEnd || trip.color} 100%)`,
              animationDelay: `${i * 0.15}s`,
            }}
          >
            <div className="trip-card-pattern" aria-hidden="true" />
            <div className="trip-card-content">
              <div className="trip-card-icon" aria-hidden="true">{trip.icon}</div>
