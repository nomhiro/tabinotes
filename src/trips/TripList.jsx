    ...
    <div style={{ fontFamily: "'Noto Serif JP', 'Hiragino Mincho ProN', serif", background: "#F7F3ED", minHeight: "100vh", color: "#2C2421" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .trip-grid { max-width:800px; margin:0 auto; padding:1rem 1.5rem 4rem; display:grid; gap:1.5rem; }
        .trip-card { position:relative; border-radius:12px; overflow:hidden; cursor:pointer; transition:transform .3s,box-shadow .3s; min-height:220px; display:flex; flex-direction:column; align-items:center; justify-content:center; text-decoration:none; color:white; animation:fadeUp .8s ease-out both; }
        ...
        .trip-card-title { font-size:clamp(1.5rem,4vw,2rem); font-weight:700; letter-spacing:.12em; margin-bottom:.3rem; text-shadow:0 2px 15px rgba(0,0,0,.2); }
        ...
        @media (max-width:500px) { .trip-grid{padding:1rem 1rem 3rem} .trip-card{min-height:180px} }
      `}</style>
